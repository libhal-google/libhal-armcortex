...
#include <functional>
#include <libarmcortex/peripherals/dwt_counter.hpp>
...
class SystemTimer final : public sjsu::SystemTimer
{
 public:
  ...
  explicit SystemTimer(sjsu::ResourceID id, int priority = -1)
  ...
  void ModuleInitialize() override
  {
    ...
    sjsu::InterruptController::GetPlatformController().Enable({
        ...
        .priority                 = priority_,
        .interrupt_vector         = SystemTimerHandler,
    });

    // Set all flags required to enable the counter
    uint32_t ctrl_mask = (1 << ControlBitMap::kClkSource) |
                         (1 << ControlBitMap::kTickInterupt) |
